...
function appJWT({ appId, privateKey }) {
  // GitHub hands you PKCS#1. node:crypto accepts it, but Web Crypto does not,
  // and the failure there is an opaque DataError at request time in production.
  // If this ever moves to an edge runtime, convert with:
  //   openssl pkcs8 -topk8 -inform PEM -outform PEM -nocrypt -in key.pem
  // Never paste an App private key into a web-based converter: it is commit
  // access to the assets repo.
  if (keyFormat(privateKey) === 'unknown')
    throw new Error('GH_APP_PRIVATE_KEY is not a PEM private key');
...
// Installation tokens last an hour. Minting one per submission burns a separate
// secondary limit of 2,000 access-token requests per hour, so cache it.
let cached = { token: null, until: 0 };

/** Exchange the App JWT for an installation token, which is what can write. */
export async function installationToken(cfg, { force = false } = {}) {
  const now = Date.now();
  if (!force && cached.token && now < cached.until) return cached.token;
  ...
  cached = { token: t.token, until: now + 50 * 60 * 1000 }; // 50 min, GitHub expires at 60
  return t.token;
}

// GitHub: "To avoid exceeding secondary rate limits, you should make requests
// serially instead of concurrently." Every mutating sequence goes through here.
let chain = Promise.resolve();
const serial = fn => (chain = chain.then(fn, fn)).catch(() => {}) && chain;

/**
 * Commit a submission onto a new branch and open a PR.
 * files: [{ path, content }] where content is a Buffer or a string.
 */
export async function submitToBranch(cfg, opts) {
  return serial(() => commitAndOpenPR(cfg, opts));
}

async function commitAndOpenPR(cfg, { branch, message, files, prTitle, prBody, coAuthor }) {
  ...
  // Omitting base_tree does not create an empty diff, it deletes every file in
  // the repository in one commit. Documented behaviour, so assert rather than trust.
  if (!baseSha) throw new Error('refusing to create a tree without base_tree');
  const newTree = await gh(`${base}/git/trees`, {
    token, method: 'POST', body: { base_tree: baseSha, tree },
  });

  // Attribution goes in a trailer, not the author field. Setting a custom author
  // silently forfeits the Verified badge, which only surfaces later when a branch
  // protection rule requiring signed commits starts rejecting the bot's PRs.
  const full = coAuthor ? `${message}\n\nCo-authored-by: ${coAuthor}` : message;
  const commit = await gh(`${base}/git/commits`, {
    token, method: 'POST', body: { message: full, tree: newTree.sha, parents: [baseSha] },
  });
