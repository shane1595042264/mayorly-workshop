...
export async function validate(bytes, spec) {
  ...
  // Attachment points. A slot may declare bays: rectangles the game draws other
  // sprites into (the ledger shelf holds nine icon.ledger.closed spines). The
  // artist owns every pixel outside the bays; inside, the art must be a flat
  // back panel or the placed sprite will sit on top of detail that shows
  // through around it. Both rules are structural, so both block.
  if (spec.bays && Array.isArray(spec.bays.rects)) {
    const { rects, spine = [0, 0], perBay = 1, holds = 'a sprite' } = spec.bays;
    const minW = perBay * spine[0] + (perBay + 1), minH = spine[1] + 1;
    const small = rects.filter(([, , bw, bh]) => bw < minW || bh < minH);
    add('bays.fit', small.length === 0, `Each bay fits ${perBay} x ${spine[0]}x${spine[1]} ${holds}`,
      small.length ? `${small.length} too small, min ${minW}x${minH}` : `${rects.length} bays`, false,
      `A bay must be at least ${minW}x${minH} for ${perBay} spines with a pixel between. Widen it in slots/${spec.id || '<slot-id>'}.json or thin the frame around it.`);
    if (a.opaque > 0) {
      const bad = [];
      rects.forEach(([bx, by, bw, bh], i) => {
        const cols = new Set(); let clear = 0;
        for (let y = by; y < by + bh; y++) for (let x = bx; x < bx + bw; x++) {
          if (x >= m.w || y >= m.h) { clear++; continue; }
          const p = (y * m.w + x) << 2;
          if (P.px[p + 3] === 0) clear++; else cols.add(P.px[p] << 16 | P.px[p + 1] << 8 | P.px[p + 2]);
        }
        if (clear || cols.size > 3) bad.push(`bay ${i + 1} ${clear ? clear + ' clear px' : cols.size + ' colours'}`);
      });
      add('bays.flat', bad.length === 0, 'Bays are a flat back panel', bad.length ? bad.join(', ') : `${rects.length} flat`, false,
        `The game draws ${holds} over each bay, so keep the bay a plain back panel: fully opaque, at most 3 colours. Put the detail on the frame around it.`);
    }
  }
